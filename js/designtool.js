...
function validate_input() {
  var warning = "";
  if (plenumConfig === "")
    warning = warning + "Please select a plenum configuration.\n";
  if ((intRoomHeight > 40) | (intRoomHeight < 1) || isNaN(intRoomHeight))
    warning = warning + "The interior room height is invalid.\n";
  if (isNaN(intFloorArea) || intFloorArea <= 0)
    warning = warning + "The interior floor area is invalid.\n";
  if (intFloorLevel === "")
    warning = warning + "Please select the interior floor level.\n";
  if (intDiffType === "")
    warning = warning + "Please select the interior diffuser type.\n";
  if (intNumDiffs < 1 || isNaN(intNumDiffs))
    warning = warning + "The number of interior diffusers is invalid.\n";
  if (intCoolingLoadMixed < 0 || isNaN(intCoolingLoadMixed))
    warning = warning + "The interior cooling load is invalid.\n";
  if ((intOccupiedT > 90) | (intOccupiedT < 60) || isNaN(intOccupiedT))
    warning = warning + "The design occupied zone temperature is invalid.\n";
  if (intLeakage > 5 || isNaN(intLeakage))
    warning = warning + "The interior leakage is invalid.\n";
  if (
    plenumConfig !== "Reverse Series" &&
    ((intAhuT < 40) | (intAhuT > intOccupiedT) || isNaN(intAhuT))
  ) {
    warning =
      warning + "The setpoint temperature in the interior plenum is invalid.\n";
  }
  if ((perRoomHeight > 40) | (perRoomHeight < 1) || isNaN(perRoomHeight))
    warning = warning + "The perimeter room height is invalid.\n";
  if (isNaN(perFloorArea))
    warning = warning + "The perimeter floor area is invalid.\n";
  if (perFloorLevel === "")
    warning = warning + "Please select the perimeter floor level.\n";
  if (perDiffType === "")
    warning = warning + "Please select the perimeter diffuser type.\n";
  if (perNumDiffs < 1 || isNaN(perNumDiffs))
    warning = warning + "The number of perimeter diffusers is invalid.\n";
  if (perCoolingLoadMixed < 0 || isNaN(perCoolingLoadMixed))
    warning = warning + "The perimeter cooling load is invalid.\n";
  if ((perOccupiedT > 90) | (perOccupiedT < 60) || isNaN(perOccupiedT))
    warning = warning + "The design occupied zone temperature is invalid.\n";
  if (perLeakage > 5 || isNaN(perLeakage))
    warning = warning + "The perimeter leakage is invalid.\n";
  if (
    (plenumConfig !== "Series") &
    (plenumConfig !== "Common Plenum") &
    ((perAhuT < 40) | (perAhuT > perOccupiedT) | isNaN(perAhuT))
  ) {
    warning =
      warning +
      "The setpoint temperature in the perimeter plenum is invalid.\n";
  }
  if (numOccupants <= 0 || isNaN(numOccupants))
    warning = warning + "The number of occupants is invalid.\n";
  if (zoneOrientation === "")
    warning = warning + "Please select the perimeter zone orientation.\n";
  if (
    (extWallLength < 1) | (extWallLength > intFloorArea) ||
    isNaN(extWallLength)
  )
    warning = warning + "The exterior wall length is invalid.\n";
  if (warning !== "") {
    alert(warning);
    return true;
  } else {
...
function get_per_phi(perGamma, perDiffType) {
  if (perDiffType === "VAV Directional Per.") {
    if (perGamma > 15) {
      ...
      perPhi67 = 0.7168 + 0.019 * perGamma;
    } else {
      if (blinds === "down") {
        perPhi4 = 0.5452 - 0.13;
      } else {
    ...
    perDiffType === "Linear Bar Grille CBE" ||
    perDiffType === "Linear Bar Grille RP"
  ) {
    if (perGamma > 23) {
      if (blinds === "down") {
        perPhi4 = 1.1074 - 0.13;
      ...
      if (blinds === "down") {
        perPhi4 =
          0.1282 + 0.0908 * perGamma - 0.0021 * Math.pow(perGamma, 2) - 0.13;
      } else {
...
function intAvgOZT_calc(intAirflowRate) {
  const intAirflowRateM = intAirflowRate * 0.0004719474; // m^3/s

  intGamma =
    Math.pow(intAirflowRateM * intCosDiffAngle, 1.5) /
    (Math.pow((intNumDiffs * intDiffEffArea) / numOccupants, 1.25) *
      numOccupants *
  ...
  intSetpointT =
    ((intReturnT - int67inT) / (intRoomHeightIn - 67)) *
      (48 - intRoomHeightIn) +
    intReturnT;
  if (intSetpointT < int4inT) intSetpointT = int4inT;
  intAvgOZT =
    (((int67inT + intSetpointT) * (67 - 48)) / 2 +
      ((int4inT + intSetpointT) * (48 - 4)) / 2) /
    (67 - 4);
...
function perAvgOZT_calc(perAirflowRate) {
  const perAirflowRateM = perAirflowRate * 0.0004719474; // m^3/s
  perGamma =
    (perAirflowRateM * perCosDiffAngle) /
    (perNumDiffs *
      perDiffEffArea *
      Math.pow(
        (0.0281 * perZF * perCoolingLoadUFADW) /
          (1000 * 0.3048 * extWallLength),
        0.3333333
      ));
  ...
  perSetpointT =
    ((perReturnT - per67inT) / (perRoomHeightIn - 67)) *
      (48 - perRoomHeightIn) +
    perReturnT;
  if (perSetpointT < per4inT) perSetpointT = per4inT;
  perAvgOZT =
    (((per67inT + perSetpointT) * (67 - 48)) / 2 +
      ((per4inT + perSetpointT) * (48 - 4)) / 2) /
    (67 - 4);
...
function calc_int_zoneT_indep(intAirflowRate) {
  ...
  intDiffT =
    intSupplyPlenumT + (3.412 * intSPCoolingLoad) / (1.08 * intDesAirflowRate);
  intReturnT =
    intDiffT + (3.412 * intZCoolingLoad) / (1.08 * intDesAirflowRate);
  intReturnPlenumT =
    intReturnT + (3.412 * intRPCoolingLoad) / (1.08 * intDesAirflowRate);
...
function calc_per_zoneT_indep(perAirflowRate) {
  ...
  perDiffT =
    perSupplyPlenumT +
    (3.412 * (perSPCoolingLoad - infloorCap)) / (1.08 * perDesAirflowRate);
  perReturnT =
    perDiffT + (3.412 * perZCoolingLoad) / (1.08 * perDesAirflowRate);
  perReturnPlenumT =
    perReturnT + (3.412 * perRPCoolingLoad) / (1.08 * perDesAirflowRate);
...
function calc_int_zoneT_reverse(intAirflowRate) {
  ...
  perDiffT =
    perSupplyPlenumT +
    (3.412 * (perSPCoolingLoad - infloorCap)) / (1.08 * totDesAirflowRate);
...
function calc_int_zoneT_series(intAirflowRate) {
  ...
  intSupplyPlenumT = intAhuT;
  intDiffT =
    intSupplyPlenumT + (3.412 * intSPCoolingLoad) / (1.08 * totDesAirflowRate);
  intReturnT =
    intDiffT + (3.412 * intZCoolingLoad) / (1.08 * intDesAirflowRate);
  intReturnPlenumT =
    intReturnT + (3.412 * intRPCoolingLoad) / (1.08 * intDesAirflowRate);
...
function calc_int_zoneT_common(intAirflowRate) {
  ...
  intDiffT =
    intSupplyPlenumT +
    (3.412 * (intSPCoolingLoad + perSPCoolingLoad)) /
      (1.08 * totDesAirflowRate);
  intReturnT =
    intDiffT + (3.412 * intZCoolingLoad) / (1.08 * intDesAirflowRate);
  intReturnPlenumT =
    intReturnT + (3.412 * intRPCoolingLoad) / (1.08 * intDesAirflowRate);
...
function calc_per_zoneT_common(perAirflowRate) {
  ...
  perDiffT =
    intSupplyPlenumT +
    (3.412 * (intSPCoolingLoad + perSPCoolingLoad - infloorCap)) /
      (1.08 * totDesAirflowRate);
  perReturnT =
    perDiffT + (3.412 * perZCoolingLoad) / (1.08 * perDesAirflowRate);
  perReturnPlenumT =
...
function toggleUnits() {
  ...
  if (SI) {
    ...
    $(
      'input[name="intOccupiedT"],input[name="perOccupiedT"],input[name="intSupplyPlenumT"], input[name="perSupplyPlenumT"]'
    ).each(function () {
    ...
    $(
      'input[name="intRoomHeight"],input[name="perRoomHeight"], input[name="extWallLength"]'
    ).each(function () {
    ...
    $('input[name="intFloorArea"], input[name="perFloorArea"]').each(
      function () {
    ...
    $(
      'input[name="intCoolingLoadMixed"], input[name="perCoolingLoadMixed"], input[name="infloorCap"]'
    ).each(function () {
...
function handle_output() {
  ...
  if (intDiffType === "Swirl" && intAirflowPerDiff > 76) {
    ...
    intDiffType === "VAV Directional Int." &&
    intAirflowPerDiff > 151
  ) {
    $("#intA3").css("color", "red");
  } else if (intDiffType === "Displacement" && intAirflowPerDiff > 91) {
    // 58: design max; 91 tested max;
    $("#intA3").css("color", "red");
  } else {
  ...
  if (
    perDiffType === "Linear Bar Grille CBE" &&
    perAirflowPerDiff > 246 * (0.0254 * barGrilleLength)
  ) {
    $("#perA3").css("color", "red");
  } else if (
    perDiffType === "Linear Bar Grille RP" &&
    perAirflowPerDiff > 490 * (0.0254 * barGrilleLength)
  ) {
    $("#perA3").css("color", "red");
  } else if (
    perDiffType === "VAV Directional Per." &&
    perAirflowPerDiff > 151
  ) {
    $("#perA3").css("color", "red");
  } else {
...
function draw_charts() {
  if (plenumConfig === "Common Plenum" || plenumConfig === "Series")
    perSupplyPlenumT = intDiffT;
...
function main() {
  ...
  if (perDiffType === "VAV Directional Per.") {
    ...
    perDiffAngle = 15;
    perDiffEffArea = 0.024; //unit is m2
  }
  ...
  if (plenumConfig === "Independent Plenums") {
    ...
    intAirflowRateOut = bisect_zoneT(
      0.001,
      100 * intFloorArea,
      calc_int_zoneT_indep,
      0.0001,
      intOccupiedT
    );
    if (intAirflowRateOut === 0) intAvgOZT = calc_int_zoneT_indep(0);
    if (intAirflowRateOut === false) return true;
    perAirflowRateOut = bisect_zoneT(
      0.001,
      100 * perFloorArea,
      calc_per_zoneT_indep,
      0.0001,
      perOccupiedT
    );
...
var perDiff = [
  ...
  },
  {
    ...
    selected: false,
    description:
      "Walnut CLEMIT-3-27-06-015-L6F: <br> (User specified length) ft x 6 in.",
